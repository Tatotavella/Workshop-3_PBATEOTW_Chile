...
void Pair3spn2::compute(int eflag, int vflag)
{
      ...
      if (iflag) {
        if (rsq < cutsq[itype][jtype]) {
            ...
            if (rsq < cut_ljsq[itype][jtype])
            {
                ...
                if (bpflag)
                {
                    // Initialize an instance of the base pair class
                    BasePair basepair;
                    ...
                    for (int q = 0; q < SIX; q++)
                    {
                        site_type[q] = (atom->type[index[q]]- 3) % 4; 
                    }
                   
                    // Initializing the distances used for cross stacking
                    basepair.init(index, site_type ,param, angle, x);
  ...
  //if (evflag) ev_tally_3spn2(eexcl, esolv,ebp, ecstk,ecoul, nbps);
...
void Pair3spn2::settings(int narg, char **arg)
{
  ...
  int i,j,k;
  int n = 14; // Hardcoded number of sites

    /* Args:
    Temperature - 
    Salt Concentration (mM)
    Short - range cutoff
    scale factor applied to Santalucia parameters
    scale factor applied to epsilon to get AT base pairing strength
    ratio between G:C and A:T base pairing
    (optional) fixID controlling the temperature of simulation
    */

  temp = force->numeric(arg[0]);
  salt_conc = force->numeric(arg[1]) / 1000.0; // Convert to [M]
  ...
  cut_lj_global = force->numeric(arg[2]);
  ...
  if (narg == 4) tmp_name = strdup(arg[3]);
...
param[1][3][3] = 6.453;
 ...
 avg_bp_energy = ref_hbond_energy * (1.0 + bp_ratio) / 2.0;
    ...
    param[11][0][0] = EMPTY;
    param[11][0][1] = ref_hbond_energy;
    param[11][0][2] = EMPTY;
    param[11][0][3] = EMPTY;
    param[11][1][0] = ref_hbond_energy;
    param[11][1][1] = EMPTY;
    param[11][1][2] = EMPTY;
    param[11][1][3] = EMPTY;
    param[11][2][0] = EMPTY;
    param[11][2][1] = EMPTY;
    param[11][2][2] = EMPTY;
    param[11][2][3] = bp_ratio * ref_hbond_energy;
    param[11][3][0] = EMPTY;
    param[11][3][1] = EMPTY;
    param[11][3][2] = bp_ratio * ref_hbond_energy;
    param[11][3][3] = EMPTY;
   ...
   avg_cstk /= 32.0;
    ...
    cstk_scale_factor = avg_bp_energy * 100/88.0 * 0.12/avg_cstk;
